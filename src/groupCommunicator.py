...
class GroupCommunicator:
    def __init__(self, conditional: threading.Condition) -> None:
        ...
        self.__periodicSendPeerMessageThread = threading.Thread(
            target=self.periodicallySendPeerMessage, args=(conditional,))
        self.__periodicReadMessageQueueThread = threading.Thread(
            target=self.processMessageQueue, args=(conditional,))
        self.__periodicallyPurgeInactivePeersThread = threading.Thread(
            target=self.periodicallyPurgeInactivePeers, args=(conditional,))
        ...
        while not self.__shutdown:
            while len(self.__UDPServer.messageQueue): #process all messages in the queue if there is any
                ...
                if message.type == "snip":
                    ...
                    conditional.acquire()
                    conditional.notifyAll() #wake up all sleeping threads so they can shutdown
                    conditional.release()
                    loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(loop)
                    loop.run_until_complete(self.__registryCommunicator.start()) #do the final registry communication
            conditional.acquire()
            conditional.wait(timeout=1.0) #check the message queue every 1 second
            conditional.release()
    ...
    def periodicallySendPeerMessage(self, conditional: threading.Condition) -> None:
        while not self.__shutdown:
            ...
            conditional.acquire()
            conditional.wait(timeout=30.0) #send peer messages every 30 seconds
            conditional.release()
    ...
    def periodicallyPurgeInactivePeers(self, conditional: threading.Condition) -> None:
        while not self.__shutdown:
            ...
            conditional.acquire()
            conditional.wait(timeout=180.0) #check again in 3 minutes
            conditional.release()
